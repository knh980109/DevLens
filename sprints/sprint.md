...
```
- [ ] `frontend/vite.config.js` 프록시 설정
```js
server: {
  proxy: {
...
## 작업
- [ ] `backend/Data/pull_requests.json` 생성
```json
[
...
```
- [ ] `backend/Data/developers.json` 생성
```json
[
...
```
- [ ] `backend/Data/ai_insights.json` 생성
```json
[
...
```
- [ ] `Models/PullRequest.cs`, `Developer.cs`, `AiInsight.cs` 생성
- [ ] `Services/MockDataService.cs` - JSON 파일 읽어서 List로 반환
- [ ] `Controllers/DashboardController.cs` - 4개 API 엔드포인트 구현
- [ ] `http://localhost:5062/api/v1/overview` 응답 확인
- [ ] 완료 후 sprint_3.md 로 이동
...
## 작업
- [ ] `frontend/src/assets/styles/_variables.scss` 생성
```scss
$primary: #6366f1;
...
```
- [ ] `frontend/src/assets/styles/main.scss` - 전역 스타일 + CSS 변수 (다크/라이트)
- [ ] `App.vue` - 다크모드 토글 로직 (localStorage 저장)
- [ ] `components/common/AppHeader.vue` - 네비게이션 + 다크모드 토글 버튼
- [ ] `router/index.js` - 4개 라우트 + 페이지 전환 트랜지션 설정
- [ ] `App.vue` 에 fade-slide 트랜지션 적용
```vue
...
```
- [ ] SCSS에 fade-slide 트랜지션 작성
- [ ] 완료 후 sprint_4.md 로 이동
...
## 작업
- [ ] `stores/dashboard.js` Pinia 스토어 생성
...
- [ ] `components/common/StatCard.vue` 생성
...
- [ ] StatCard 카운터 애니메이션 동작 확인
- [ ] 완료 후 sprint_5.md 로 이동
...
## 작업
- [ ] `views/OverviewView.vue` 완성
  - StatCard 4개 (전체 PR, 오픈, 머지, 평균 품질점수)
  - 주간 PR 추이 LineChart
  - 평균 리뷰 소요시간 GaugeChart
  - 개발자별 PR 수 BarChart
- [ ] `components/charts/LineChart.vue` - ApexCharts 라인 차트
- [ ] `components/charts/GaugeChart.vue` - ApexCharts 반원 게이지
- [ ] API 연동 (fetchOverview, fetchPullRequests 호출)
- [ ] 완료 후 sprint_6.md 로 이동
...
- [ ] `components/pr/PrDetailPanel.vue` 생성
  - 오른쪽에서 슬라이드인 패널
  - PR 상세 정보 + AI 코드 품질 분석 요약
  - ESC 키 / 바깥 클릭 시 닫기
- [ ] 완료 후 sprint_7.md 로 이동
...
## 작업
- [ ] `views/DeveloperView.vue` 완성
  - 개발자 카드 그리드 (이름, 역할, 총 PR, 머지율, 평균 품질점수)
  - 카드 클릭 시 상세 모달 오픈
  - 상세 모달: RadarChart + 통계 수치
- [ ] `components/charts/RadarChart.vue` - ApexCharts 레이더 차트
  - 5개 축: 속도 / 품질 / 협업 / 일관성 / 커버리지
- [ ] 개발자 카드 hover 애니메이션 (transform: translateY)
- [ ] 완료 후 sprint_8.md 로 이동
...
## 작업
- [ ] 전체 페이지 SCSS 반응형 적용 (1280px / 768px 브레이크포인트)
- [ ] 다크모드 전체 페이지 검수
- [ ] 페이지 전환 트랜지션 자연스럽게 조정
- [ ] 로딩 스피너 모든 페이지 적용 확인
- [ ] 빈 데이터 Empty State UI 처리
- [ ] 전체 흐름 시연 테스트
  - Overview → PR 목록 → 상세 패널 → 개발자 분석 → AI 인사이트
- [ ] 완료 후 sprint_10.md 로 이동
...
## 작업
- [ ] `docs/` 폴더 생성 후 화면 스크린샷 저장
- [ ] README.md 스크린샷 경로 확인
- [ ] `.gitignore` 확인 (node_modules, bin, obj, .env 제외)
- [ ] GitHub push
```powershell
git init
git add .
